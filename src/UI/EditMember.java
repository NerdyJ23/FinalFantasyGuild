...
/**
 *  Form for editing an existing Member (Staff or Player)
 * @author Jessica Moolenschot
 */
public class EditMember extends javax.swing.JFrame implements ItemListener,WindowListener //honestly could have reused the add member and passed a boolean but i'm in a rush soooo
{
    ...
    DAL database;
    /**
     * Initializes the EditMember form
     */
    ...
    /**
     * Pre-select the guild based on the existing Members Guild
     * @param guildID GuildID
     */
    public void selectGuild(int guildID)
    {
        int count = 0;
            
            for(Guild g : database.getGuilds())//iterate through guilds until guildID matches, use count for list pos (lazy hack)
            {
                if(g.getGuildID() == guildID)
                {
                    ddlGuild.setSelectedIndex(count);
    ...
    /**
     * Pre-select the classRole based on the existing Members classRole
     * @param role classRole
     */
    public void selectClassRole(String role)
    {
        for(int a = 0; a < ddlRole.getItemCount(); a++)
        {
            if(ddlRole.getItemAt(a).equals(role))
            {
                ddlRole.setSelectedIndex(a);
                break; //end loop early
            }
        }
    }
    /**
     * Pre-select the Region JRadioButton based on the existing Members region
     * @param region 
     */
    ...
    {    
        ...
        {
            case "Japan":
                rdJapan.setSelected(true);
                break;
            case "America":
                rdAmerica.setSelected(true);
                break;
            case "Europe":
                rdEurope.setSelected(true);
                break;
        }
    }
    /**
     * Pre-selects the DataCenter based on the existing Members datacenter
     * @param datacenter DataCenter
     */
    public void selectDataCenter(String datacenter)
    {            
        for(int a = 0; a < ddlDataCenter.getItemCount(); a++)//iterate through the datacenter values 
        {
            if(ddlDataCenter.getItemAt(a).equals(datacenter))
            {
                ddlDataCenter.setSelectedIndex(a);//select the index if it's the same name
                break; //end loop early
            }
        }
    }
    /**
     * Pre-select the Server based on the existing Members server
     * @param serverName 
     */
    public void selectServer(String serverName)
    {            
        for(int a = 0; a < serverList.getModel().getSize(); a++)
        {
            if(serverList.getModel().getElementAt(a).equals(serverName))
            {
    ...
    /**
     * Pre-fill the fields based on the Members existing information
     */
    public void prefillFields()
    {
        Object o = database.getMemberByID(memberID);
        if(o.getClass() == Player.class)
        {
            Player temp = (Player)o;
            Trace.log("it was a player!!!");
            txtFirstName.setText(temp.getFirstName());
            txtLastName.setText(temp.getLastName());
            
            txtCharName.setText(temp.getCharacterName());
            selectGuild(temp.getGuildID());
            selectClassRole(temp.getCharacterRole());
            selectRegion(temp.getRegion());
            selectDataCenter(temp.getDatacenter());
            selectServer(temp.getServer());
            
            List<String> list = ParseClass.parseClasses(temp.getCharacterClass());
            int count = 0;
            for(Component c: ClassPanel.getComponents())
            {
                if(c instanceof JCheckBox)
                {
                    ...
                    for(String s: list)
                    {
                        String[] className = s.toLowerCase().split("_");
                        String output = "";
                        for(String name : className)
                        {
                            output+= Strings.Strings.capitalize(name) + " ";
                        }
                        if(output.trim().equals(((JCheckBox) c).getText()))
                        {
                            ((JCheckBox) c).setSelected(true);
                            Trace.log("Checkbox " + ((JCheckBox) c).getText() + " set to selected");
                            break;//end list parse early
                        }
                    }
                     //item.getText();
                }
            }
            Trace.log("there were " + count + " checkboxes");
            //serverList.setSelectedIndex();
            //ddlDataCenter.setSelectedIndex();
            //ddlGuild.setSelectedItem(o);
        }
        else
        {
            Staff temp = (Staff)o;
            ...
            selectDataCenter(temp.getDatacenter());
            selectServer(temp.getServer());
            for(int a = 0; a < ddlStaffRole.getItemCount(); a++)//iterate through the staff role values 
            {
                String name = Staff.Role.values()[a].name();
                
                if(name.equals(temp.getRole()))
                {
                    ddlStaffRole.setSelectedIndex(a);//select the index if it's the same name
                    break; //end loop early
                }
            }
        }
    ...
    {//GEN-HEADEREND:event_btnAddActionPerformed
        if(validateInputs())//all inputs are valid
        {
            ...
            if(isStaff)
            {
                ...
                if(database.updateMember(s))
                {
                    JOptionPane.showMessageDialog(this, "Updated successfully!", "Success!", JOptionPane.INFORMATION_MESSAGE);
                    this.dispose();
                }
                else
                    JOptionPane.showMessageDialog(this, "Could not update member!", "Error", JOptionPane.ERROR_MESSAGE);
...
//                            }
    ...
    @Override
    /**
     * Hide/show fields depending on whether the user is editing a Player or Staff Member and update the DataCenter JComboBox with Servers that correspond to the selected Region
     */
